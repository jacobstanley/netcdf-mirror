...
static int extract(NCDAPCOMMON*, Getvara*, CDFnode*, DCEsegment*, OClink, OCdatanode, struct NCMEMORY*);
static int extractstring(NCDAPCOMMON*, Getvara*, CDFnode*, DCEsegment*, OClink, OCdatanode, struct NCMEMORY*);
...
#endif
	    /* Create an odometer for this structures' part of the projection */
            odom = dapodom_fromsegment(segment,rank);
	...
	if(fIsSet(mode,OCDT_SEQUENCE)) {
	    ...
	    /* Build an odometer for walking the sequence,
            ...
            */
            ...
            odom = dapodom_fromsegment(segment,segment->rank);
	    while(dapodom_more(odom)) {
...
fprintf(stderr,"\n");
    ...
    } else if(xgetvar->cache->wholevariable) {/* && rank > 0 */
	/* There are multiple cases, assuming no conversion required.
	      ...
	      => read whole subset at one shot
           3) client is asking for strided subset
              => start=x, count=y, stride=s
	      => we have to use odometer on leading, strided prefix.
           If conversion required, then read one-by-one
	*/
	int safeindex = dcesafeindex(segment);

	if(!requireconversion && safeindex == 0) { /* can read whole thing */
	    size_t internlen;
	    count = dcesafesegmentsize(segment); /* how many to read */
	    ...
	    memory->next += internlen;
            if(ocstat != OC_NOERR) {THROWCHK(ocstat); goto done;}
	} else if(!requireconversion && safeindex > 0) {
	    size_t internlen;
	    /* We need to build an odometer for the unsafe prefix of the slices */
	    odom = dapodom_fromsegment(segment,safeindex);
	    count = dcesafesegmentsize(segment); /* read in count chunks */
	    internlen = interntypesize*count;
	    while(dapodom_more(odom)) {
	        ocstat = oc_data_readn(conn,currentcontent,odom->index,count,internlen,value);
	        if(ocstat != OC_NOERR) {THROWCHK(ocstat); goto done;}
	        memory->next += internlen;
	        dapodom_next(odom);
	    }
	     ...
	     /* Cover following cases, all of which require reading
                values one-by-one:
		1. requireconversion
		2. !requireconversion but safeindex == -1 =>no safe indices
    ...
    } else { /* !xgetvar->cache->wholevariable && rank > 0 */
	...
	*/
	if(requireconversion) {
	    ...
	    count = dcesafesegmentsize(segment); /* how many to read */
            ocstat = oc_data_readn(conn,currentcontent,dap_zero,count,interntypesize,memory->next);
            if(ocstat != OC_NOERR) {THROWCHK(ocstat); goto done;}
	}
    }
done:
       ...
       )
{
    ...
    if(rank == 1) {/*=> scalar because the single dimension is stringdim */
	char* value = NULL;
	...
	nclistpush(strings,(ncelem)value);	
        ...
        /* Since we have a string dim, reduce the rank */
        odom = dapodom_fromsegment(segment,rank-1);
        while(dapodom_more(odom)) {
	    char* value = NULL;
