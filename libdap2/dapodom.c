...
Dapodometer*
dapodom_fromsegment(DCEsegment* segment, size_t pseudorank)
{
    int i;

    Dapodometer* odom = (Dapodometer*)calloc(1,sizeof(Dapodometer));
    MEMCHECK(odom,NULL);
    odom->rank = pseudorank;
    assert(odom->rank <= NC_MAX_VAR_DIMS);
    for(i=0;i<odom->rank;i++) {
	odom->start[i] = segment->slices[i].first;
	odom->count[i] = segment->slices[i].count;
	odom->stride[i] = segment->slices[i].stride;
	odom->size[i] = segment->slices[i].declsize;
...
{
    return (odom->index[0] < odom->start[0] + odom->count[0]);
}
...
dapodom_next(Dapodometer* odom)
{
    ...
    for(i=odom->rank-1;i>=0;i--) {
        ...
        if(odom->index[i] < odom->size[i]) break;
